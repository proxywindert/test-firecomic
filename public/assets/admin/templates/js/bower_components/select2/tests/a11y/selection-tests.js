...
test('the container should be in the tab order', function (assert) {
  ...
  assert.equal(
    ...
    '0',
    'The tab dashboard should allow it to fit in the natural tab order'
  );
  ...
  assert.equal(
    $selection.attr('tabindex'),
    '0',
    'The tab dashboard should be restored when re-enabled'
  );
});
...
test('a custom tabindex is copied', function (assert) {
  ...
  assert.equal(
    $selection.attr('tabindex'),
    '999',
    'The tab dashboard should match the original tab dashboard'
  );
